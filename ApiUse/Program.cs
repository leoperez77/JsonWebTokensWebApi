...
namespace ApiUse
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                var postValues = new NameValueCollection { { "grant_type", "password" },{ "username", "superuser" } };
                ...
                byte[] result = asClient.UploadValues("http://koalaauthapi.azurewebsites.net/oauth2/token", "POST", postValues);
            ...
            using (var rsClient = new WebClient())
            {
                ...
                byte[] result = rsClient.DownloadData("http://localhost:54404/api/protected");
                string receivedResponseData = Encoding.UTF8.GetString(result);
                Console.Write(receivedResponseData);
